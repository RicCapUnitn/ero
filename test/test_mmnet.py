...
class TestNetworkImport(unittest.TestCase):
    ...
    def test_import_events(self):
        ...
        # Fill the person dictionary with some person, it is not important which one
        person = Person([])
        number_of_people = self.ero.mmnet.mmnet.GetModeNetByName("Person").GetNodes()
...
class TestPropagation(unittest.TestCase):
    ...
    def test_propagation_multiple_events_one_iteration_single_overlap(self):
        ...
        event_mode = mmnet.mmnet.GetModeNetByName("Event")
        event_mode.AddNode(event_1.id)
        event_mode.AddNode(event_2.id)

        event_to_person_crossnet = mmnet.mmnet.GetCrossNetByName(
            "EventToPerson")
        edge_id_1 = event_to_person_crossnet.AddEdge(
            event_1.id, propagating_node_1)
        edge_id_2 = event_to_person_crossnet.AddEdge(
            event_2.id, propagating_node_2)
